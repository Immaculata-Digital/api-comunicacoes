...
export class UpdateRemetenteSmtpUseCase {
  constructor(private readonly remetenteSmtpRepository: IRemetenteSmtpRepository) {}

  async execute(schema: string, id: string, data: UpdateRemetenteSmtpDTO) {
    ...
    const remetente = RemetenteSmtp.restore(existing)
    const updateData: {
